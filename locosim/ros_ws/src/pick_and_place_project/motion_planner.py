...
class MotionPlanner:
    ...
    def ur5_inverse(self, p_target, R_target):
        ...
        for th1 in th1_options:
            ...
            for th3_sign in [1, -1]:
                ...
                # R01 = Rz(θ1)
                R01 = np.array(
                    [[np.cos(th1), -np.sin(th1), 0], [np.sin(th1), np.cos(th1), 0], [0, 0, 1]]
                )

                # R12 = Rx(π/2) @ Rz(θ2)
                Rx_90 = np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]])
                Rz_th2 = np.array(
                    [[np.cos(th2), -np.sin(th2), 0], [np.sin(th2), np.cos(th2), 0], [0, 0, 1]]
                )
                R12 = Rx_90 @ Rz_th2

                # R23 = Rz(θ3)
                R23 = np.array(
                    [[np.cos(th3), -np.sin(th3), 0], [np.sin(th3), np.cos(th3), 0], [0, 0, 1]]
                )
        ...
        for sol in solutions:
            ...
            if pos_error < 0.1 and orient_error < 0.5:  # 10cm, ~30 deg tolerance for candidates
                ...
                rospy.logdebug(
                    f"  Valid candidate: pos_err={pos_error*1000:.1f}mm, orient_err={np.degrees(orient_error):.1f}°"
                )
        ...
        if len(final_solutions) == 0:
            rospy.logwarn(
                f"URDF IK: No valid solutions found after validation (had {len(solutions)} candidates)"
            )
            return None
    ...
    def gripper_down_rotation(self):
        ...
        """
        R = np.array(
            [[1, 0, 0], [0, -1, 0], [0, 0, -1]]  # X-axis: forward  # Y-axis: -Y  # Z-axis: down
        )
        return R
    ...
    def move_to_joints_direct(self, target_joints, controller, duration=1.0, target_world_pos=None):
        ...
        if len(target_joints) == 6:
            ...
            interpolation_delta_full = np.concatenate(
                [interpolation_delta, target_joints_full[6:8] - current_joints[6:8]]
            )
    ...
    def pick_object(self, object_pos, controller, object_orientation=None, object_class=None):
        ...
        if object_pos[0] < -0.5 or object_pos[0] > 1.5:
            rospy.logerr(
                f"Object X={object_pos[0]:.2f} is outside valid range [-0.5, 1.5] - object likely fell off table!"
            )
            return False
        if object_pos[1] < -0.5 or object_pos[1] > 1.5:
            rospy.logerr(
                f"Object Y={object_pos[1]:.2f} is outside valid range [-0.5, 1.5] - object likely fell off table!"
            )
            return False
        if object_pos[2] < 0.5 or object_pos[2] > 1.5:
            rospy.logerr(
                f"Object Z={object_pos[2]:.2f} is outside valid range [0.5, 1.5] - object likely fell off table!"
            )
            return False
        ...
        approach_pos = np.array(
            [object_pos[0], object_pos[1], object_pos[2] + approach_height_offset]
        )

        # Compute grasp position (at object) - now using adaptive height
        grasp_pos = np.array([object_pos[0], object_pos[1], object_pos[2] + grasp_height_offset])
        ...
        self.move_to_joints(
            safe_joints, controller, duration=getattr(self.config, 'default_move_duration', 2.0)
        )
        ...
        self.move_to_joints(
            approach_joints, controller, duration=getattr(self.config, 'approach_duration', 1.8)
        )
        current_joints, _ = controller.get_current_joint_state()
        ...
        self.move_to_joints(
            grasp_joints, controller, duration=getattr(self.config, 'grasp_duration', 1.5)
        )
        ...
        if object_class:
            try:
                ...
                if obj_size is not None:
                    ...
                    rospy.logwarn(
                        f"  Object class '{object_class}' not found in BRICK_CLASSES, using default grip"
                    )
            except ImportError:
    ...
    def place_object(self, target_pos, controller):
        ...
        robot_base_x = 0.5
        robot_base_y = 0.35
        current_ee_world = np.array(
            [
                current_ee_pos[0] + robot_base_x,
        ...
        if needs_transition:
            ...
            arc_y = max(
                current_ee_world_y, target_y + 0.15, robot_base_y + 0.20
            )  # At least 15cm above target Y
        ...
        self.move_to_joints(
            ...
            controller,
            duration=getattr(self.config, 'approach_duration', 1.8),
        )
        ...
        if place_joints is None:
            ...
            return False
        self.move_to_joints(
            place_joints, controller, duration=getattr(self.config, 'grasp_duration', 1.5)
        )
    ...
    def simple_ik(self, target_pos, gripper_down=True, object_yaw=None):
        ...
        if self.config.use_analytical_ik:
            ...
            rospy.logerr(
                f"ANALYTICAL IK FAILED: Cannot reach target [{x_base:.3f}, {y_base:.3f}, {z_base:.3f}]"
            )
            return None
        ...
        for i, start_config in enumerate(starting_configs):
            result = self._numerical_ik_urdf(
                target_base, start_config, gripper_down=gripper_down, object_yaw=object_yaw
            )
        ...
        rospy.logerr(
            f"NUMERICAL IK FAILED: Cannot reach target [{x_base:.3f}, {y_base:.3f}, {z_base:.3f}]"
        )
        return None
        ...
        if gripper_down:
            ...
            if r < A2 + A3 and r > abs(A2 - A3):
                ...
                if abs(cos_t3) <= 1:
                    ...
                    theta2_est = np.arctan2(height, reach) - np.arctan2(
                        A3 * np.sin(theta3_est), A2 + A3 * np.cos(theta3_est)
                    )
    ...
    def _fast_numerical_ik(
        self, p_target, R_target, q_seed=None, max_iter=100, pos_tol=1e-4, rot_tol=1e-2
    ):
        """
    ...
    def _numerical_ik_urdf(
        self, target_base, current_joints=None, gripper_down=True, object_yaw=None
    ):
        """
        ...
        seeds = self._generate_ik_seeds(
            px, py, pz, gripper_down=gripper_down, object_yaw=object_yaw
        )
        ...
        MAX_ACCEPTABLE_JOINT_DIST = np.radians(
            300
        )  # 300 degrees total - allows base rotation to other side

        if valid_solutions:
            valid_solutions.sort(
                key=lambda x: (x[2], x[1])
            )  # Sort by joint_dist first, then pos_err
            ...
            if best_joint_dist > MAX_ACCEPTABLE_JOINT_DIST:
                ...
                rospy.logerr(
                    f"     Best solution requires {np.degrees(best_joint_dist):.0f}° total joint movement"
                )
                rospy.logerr(
                    f"     Maximum acceptable is {np.degrees(MAX_ACCEPTABLE_JOINT_DIST):.0f}°"
                )
        ...
        if best_solution is not None:
            ...
            if final_pos_error < 0.01:  # 10mm tolerance
                ...
                rospy.logwarn(
                    f"  ⚠ IK solution found but error {final_pos_error*1000:.1f}mm > 10mm"
                )
